...
public class ServiceDetail {
  ...
  **/
  ...
  @Override
  public int hashCode() {
    return Objects.hash(links, createdAt, id, tags, updatedAt, config, configs, encryptionRequired, maxIdleTimeMillis, name, permissions, postureQueries, roleAttributes, terminatorStrategy);
  }
    ...
    if (getPostureQueries() != null) {
      for (int i = 0; i < getPostureQueries().size(); i++) {
        if (getPostureQueries().get(i) != null) {
          joiner.add(getPostureQueries().get(i).toUrlQueryString(String.format("%spostureQueries%s%s", prefix, suffix,
          "".equals(suffix) ? "" : String.format("%s%d%s", containerPrefix, i, containerSuffix))));
        }
      }
    }
