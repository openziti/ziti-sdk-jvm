...
public class SessionEdgeRouter {
  ...
  **/
  ...
  @Override
  public int hashCode() {
    return Objects.hash(appData, cost, disabled, hostname, isOnline, name, noTraversal, supportedProtocols, syncStatus, urls);
  }
    ...
    if (getUrls() != null) {
      for (String _key : getUrls().keySet()) {
        joiner.add(String.format("%surls%s%s=%s", prefix, suffix,
            "".equals(suffix) ? "" : String.format("%s%d%s", containerPrefix, _key, containerSuffix),
            getUrls().get(_key), URLEncoder.encode(String.valueOf(getUrls().get(_key)), StandardCharsets.UTF_8).replaceAll("\\+", "%20")));
      }
    }
