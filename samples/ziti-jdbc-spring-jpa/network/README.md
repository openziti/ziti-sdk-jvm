...
The `startup.sh` script will pull these files for you if you haven't downloaded them already. Here is how you pull them if you prefer to
review prior to running them.

Open a terminal window to the `<repository>/samples/ziti-jdbc-spring-jpa/network` folder.  Leave this terminal open, you'll need it to manage the 
network too.
...
Run the startup script from the `<repository>/samples/ziti-jdbc-spring-jpa/network` directory:
...
To run the teardown script from the `<repository>samples/ziti-jdbc-spring-jpa/network` directory:
...
* **Services:** Services are endpoints that can be invoked over the OpenZiti network. This exercise includes two 
...
This exercise uses two OpenZiti identities:
...
```shell
ziti edge create config demo-service-config ziti-tunneler-client.v1 '{"hostname": "exercise.web","port": 8080}'
...
* **database-bind-policy:** The public edge router has the identity role `#databases`. This policy allows the edge router to host the 
`private-postgres` OpenZiti service.
...
# The End
At this point your network should be running and correctly configured for use with the jpa exercise.
